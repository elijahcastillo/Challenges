...
// Macro to append multiple items from a source array to a dynamic array
#define dyarr_append_multiple(arr, src, len)                                               \
    do {                                                                                   \
        ...
        memcpy(&(arr)->items[(arr)->size], (src), (len) * sizeof(*(arr)->items));          \
        (arr)->size += (len);                                                              \
    } while (0)
